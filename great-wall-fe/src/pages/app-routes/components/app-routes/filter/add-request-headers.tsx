...
import {addRequestHeadersFilterSchema, AddRequestHeadersFilterSchemaValues} from "@/constant/api/app-routes/schema.ts";
 ...
 * @constructor
 */
export default function AddRequestHeaders(props: RoutesProps<AddRequestHeadersFilterSchemaValues>) {
  const {
    value,
    onChange,
    onRemove,
  ...
  const form = useForm<AddRequestHeadersFilterSchemaValues>({
    resolver: zodResolver(addRequestHeadersFilterSchema),
    defaultValues: {...(value || {headers: [{name: "", value: ""}]}), type: RouteFilterEnum.AddRequestHeaders},
   ...
   * @param data
   */
  async function onSubmit(data: AddRequestHeadersFilterSchemaValues) {
    await onChange?.(data)
  }
