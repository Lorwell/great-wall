  ...
  return (
    <div {...calendarProps} className="space-y-4">
      <div className="relative flex items-center justify-center pt-1">
        <Button
          {...prevButtonProps}
          onClick={prevButtonProps.onClick as unknown as (e: React.SyntheticEvent) => void}
          ref={prevButtonRef}
          variant={"outline"}
        ...
        <Button
          {...nextButtonProps}
          onClick={nextButtonProps.onClick as unknown as (e: React.SyntheticEvent) => void}
          ref={nextButtonRef}
          variant={"outline"}
    ...
    <table {...gridProps} className={cn(gridProps.className, "w-full border-collapse space-y-1")}>
      ...
      {[...new Array(weeksInMonth).keys()].map((weekIndex) => (
        <tr className="flex w-full mt-2" key={weekIndex}>
          {state.getDatesInWeek(weekIndex).map((date, i) => (date ? <CalendarCell key={i} state={state} date={date}/> :
            <td key={i}/>))}
        </tr>
      ))}
      </tbody>
...
function CalendarCell({state, date}: CalendarCellProps) {
  ...
  return (
    ...
    >
      <Button
        {...buttonProps}
        onClick={buttonProps.onClick as unknown as (e: React.SyntheticEvent) => void}
        type="button"
        variant={"ghost"}
        ref={ref}
        className={cn(
